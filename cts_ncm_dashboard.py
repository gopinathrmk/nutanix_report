...
def get_host_stats(clusters_api,cluster):
    ...
    for host in hosts.data:
        ...
        overall_memory_usage_ppm_upper_buf = get_avg_value(host_stats.data.aggregate_hypervisor_memory_usage_ppm_upper_buf )
        overall_memory_usage_ppm_lower_buf = get_avg_value(host_stats.data.aggregate_hypervisor_memory_usage_ppm_lower_buf )
        ...
        host_info = {
            ...
            "Memory Usage (GB)" : round((aggregate_hypervisor_memory_usage_ppm/1000_000 * host.memory_size_bytes) / (GB_or_GiB ** 3),2),
...
def get_optimal_num_vms(cluster_stats_details,vm_stats_details,oc_ratio,type="demand"):
    vms_allocation = {}
    available_resources = {}
    ...
    if type == "demand":
        ...
        resource_allocated = {
            "vCPU": vm_stats_details.get("total_vms_vcpu_allocated") ,
            "memory_gb": vm_stats_details.get("total_vms_memory_gb_allocated") ,
            "disk_gb": vm_stats_details.get("total_vms_storage_gb_allocated") 
        }    
...
def get_report(vmm_api,vmm_stats_api,storage_container_api,clusters_api,cluster,category_api):
    ...
    vm_stats_details = get_vm_stats(vmm_api,vmm_stats_api,cluster,category_api)

    # for key,value in cluster_stats_details.items():
    #     print (key,value)
    
    # for key,value in vm_stats_details.items():
    #     print (key,value)

    remaining_vm_list = {"demand":{},"allocation":{} }

    for env_name,oc_ratio in environment.items():
        remaining_vm_list["demand"][env_name] = get_optimal_num_vms(cluster_stats_details,vm_stats_details,oc_ratio,type="demand")
        remaining_vm_list["allocation"][env_name] = get_optimal_num_vms(cluster_stats_details,vm_stats_details,oc_ratio,type="allocation")

    # pprint.pprint(remaining_vm_list)
    return cluster_stats_details,vm_stats_details,remaining_vm_list


def write_to_file(list_of_dict,filename,mode):
    """
...
#    password = getpass.getpass(prompt="Enter password: ")
    ...
    for cluster in clusters.data :
        if 'AOS' in cluster.config.cluster_function:
            ...
            cluster_stats_details,vm_stats_details,remaining_vm_list= get_report(vmm_api,vmm_stats_api,storage_container_api,clusters_api,cluster,category_api)
            ...
            for i  in range(3):
                ...
                report.append({
                    ...
                    "Allocated " : str(vm_stats_details.get("total_vms_"+ resource+"_allocated")) + unit + " " +
                                     str(round( vm_stats_details.get("total_vms_"+ resource+"_allocated")/cluster_stats_details.get(resource+"_capacity") * 100,2) ) + "%",
                    "Consumed/Demand " : str(cluster_stats_details.get(resource+"_used")) + unit + " " +
                                     str(round( cluster_stats_details.get(resource+"_used")/cluster_stats_details.get(resource+"_capacity") * 100,2) ) + "%",
                    "Available " : str(cluster_stats_details.get(resource+"_available")) + unit + " " +
                                     str(round( cluster_stats_details.get(resource+"_available")/cluster_stats_details.get(resource+"_capacity") * 100,2) ) + "%",
                    "Environment" : env ,
            ...
            write_to_file(list_of_dict=report,filename=filename,mode='a')
